...
class YoloDetector:
    ...
    def _postprocess(self, imgs, origimgs, pred, conf_thres, iou_thres):
        ...
        for image_id, origimg in enumerate(origimgs):
            ...
            for j in range(det.size()[0]):
                ...
                confidences[image_id].append(conf)
    ...
    def detect_faces(self, imgs, conf_thres=0.7, iou_thres=0.5):
        ...
        bboxes, confidences, points = self._postprocess(images, origimgs, pred, conf_thres, iou_thres)
        ...
        if not isListempty(points):
            ...
            return np.concatenate((bboxes, confidences, points), axis=1)
        else:
            return None
