    ...
    def __getitem__(self, index):
        ...
        if self.use_corrupt:
            ...
            if self.jpeg_range is not None:
                ...
                img_in_jpeg = np.clip((img_in * 255.0).round(), 0, 255).astype(np.uint8)
                _, encimg = cv2.imencode('.jpg', img_in_jpeg, encode_param)
        ...
        if self.use_corrupt:
            ...
            if self.jpeg_range_large is not None:
                ...
                encode_param = [int(cv2.IMWRITE_JPEG_QUALITY), int(jpeg_p)]
                img_in_large_jpeg = np.clip((img_in_large * 255.0).round(), 0, 255).astype(np.uint8)
                _, encimg = cv2.imencode('.jpg', img_in_large_jpeg, encode_param)
