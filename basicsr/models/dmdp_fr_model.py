...
class DMDPFRModel(SRModel):
    ...
    from the frozen DMGQ-VAE prior.  Stage-3: enables grain-aware fusion and
    ...
    def init_training_settings(self):
        ...
        if train_latent_gt_path is not None:
            ...
            raise NotImplementedError("DMDPFR requires network_vqgan or precomputed multi-grain latent_gt.")
    ...
    def setup_optimizers(self):
        train_opt = self.opt["train"]
        optim_g_opt = dict(train_opt["optim_g"])
        ...
        self.optimizer_g = self.get_optimizer(optim_type, optim_params_g, lr=base_lr, **optim_g_opt)
        self.optimizers.append(self.optimizer_g)
        if self.use_gan:
            optim_d_opt = dict(train_opt["optim_d"])
            optim_type = optim_d_opt.pop("type")
            base_lr_d = optim_d_opt.pop("lr")
            self.optimizer_d = self.get_optimizer(optim_type, self.net_d.parameters(), lr=base_lr_d, **optim_d_opt)
    ...
    def _generate_gt(self):
        if self.generate_idx_gt:
            ...
            if self.idx_gt is None or self.grain_gt is None:
                raise ValueError("Precomputed DMGQ latent_gt must contain both 'indices' and 'grain_indices'.")
            net_g = self._unwrap(self.net_g)
            with torch.no_grad():
    ...
    def _code_and_route_losses(self, pred, lq_feat, loss_dict):
        ...
        if self.encoder_gate_loss_weight > 0:
            if encoder_gate is None:
                raise ValueError("encoder_gate_loss_weight > 0 requires DMDPFR to return encoder_gate logits.")
        ...
        if self.hq_feat_loss:
            # LQ dynamic feature approaches the quantized HQ feature from the frozen DMGQ-VAE.
    ...
    def optimize_parameters(self, current_iter):
        ...
        with self.amp_autocast():
            if use_image_losses:
                # Stage-3 image losses train the route-aware GAMF/SFT restoration
                # branch while the DMGQ code prior stays hard and detached.
                # Code/route predictors are still optimized by their dedicated
                # CE losses below, but no straight-through image-loss path is used.
                self.output, pred, lq_feat = self.net_g(
                    train_input, w=train_w, detach_quant=True)
            else:
